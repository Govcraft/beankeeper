...
fn purchase_inventory_on_credit() {
    ...
    let txn = JournalEntry::new("Purchase inventory on credit")
...
fn owner_investment() {
    ...
    let txn = JournalEntry::new("Owner investment")
...
fn pay_rent_expense() {
    // Pay $1,200 rent
    let txn = JournalEntry::new("Monthly rent payment")
...
fn sale_on_account() {
    ...
    let txn = JournalEntry::new("Sale on account")
...
fn collect_receivable() {
    ...
    let txn = JournalEntry::new("Collect accounts receivable")
    ...
    let cash_impact = txn.amount_for_account(&cash()).unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(cash_impact, Some(Money::new(Amount::new(500_00), Currency::USD)));

    let ar_impact = txn
        .amount_for_account(&accounts_receivable())
        .unwrap_or_else(|e| panic!("{e}"));
    // Credit on debit-normal account = decrease = negative
    assert_eq!(ar_impact, Some(Money::new(Amount::new(-500_00), Currency::USD)));
}

#[test]
fn unbalanced_transaction_is_rejected() {
    let result = JournalEntry::new("Bad transaction")
...
fn mixed_currency_transaction_is_rejected() {
    let result = JournalEntry::new("Mixed currencies")
        .debit(&cash(), Money::usd(100_00))
        .unwrap_or_else(|e| panic!("{e}"))
...
fn full_accounting_cycle() {
    ...
    let txn1 = JournalEntry::new("Owner investment")
        .debit(&cash(), Money::usd(10_000_00))
        .unwrap_or_else(|e| panic!("{e}"))
        .credit(&owners_equity(), Money::usd(10_000_00))
        .unwrap_or_else(|e| panic!("{e}"))
    ...
    let txn2 = JournalEntry::new("Buy inventory")
    ...
    // 3. Cash sale: $2,000
    let txn3 = JournalEntry::new("Cash sale")
    ...
    // 4. Pay rent: $1,500
    let txn4 = JournalEntry::new("Pay rent")
    ...
    let txn5 = JournalEntry::new("Buy supplies")
    ...
    let txn6 = JournalEntry::new("Pay supplier")
    ...
    let cash_balance = ledger.balance_for(&cash()).unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(cash_balance, Amount::new(9_300_00));

    // Inventory: +3,000
    let inv_balance = ledger.balance_for(&inventory()).unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(inv_balance, Amount::new(3_000_00));

    // A/P: +3,000 credit - 1,000 debit = 2,000 (credit-normal, so positive means credit > debit)
    let ap_balance = ledger.balance_for(&accounts_payable()).unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(ap_balance, Amount::new(2_000_00));

    // Owner's equity: 10,000
    let eq_balance = ledger.balance_for(&owners_equity()).unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(eq_balance, Amount::new(10_000_00));

    // Revenue: 2,000
    let rev_balance = ledger.balance_for(&sales_revenue()).unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(rev_balance, Amount::new(2_000_00));

    // Rent expense: 1,500
    let rent_balance = ledger.balance_for(&rent_expense()).unwrap_or_else(|e| panic!("{e}"));
    assert_eq!(rent_balance, Amount::new(1_500_00));

    // Supplies expense: 200
    let sup_balance = ledger.balance_for(&supplies_expense()).unwrap_or_else(|e| panic!("{e}"));
...
fn eur_transaction() {
    ...
    let txn = JournalEntry::new("European sale")
...
fn jpy_transaction_no_minor_units() {
    ...
    let txn = JournalEntry::new("Japanese sale")
...
fn display_formatting() {
    let txn = JournalEntry::new("Display test")
        .debit(&cash(), Money::usd(100_00))
        .unwrap_or_else(|e| panic!("{e}"))
        .credit(&sales_revenue(), Money::usd(100_00))
        .unwrap_or_else(|e| panic!("{e}"))
