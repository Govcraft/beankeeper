...
pub fn run(
    ...
    if demo {
        ...
        if !cli.verbosity.quiet {
            eprintln!(
                "[ok] Populated demo data (3 companies, intercompany transactions included)"
            );
        }
    }
...
fn populate_demo_data(db: &Db) -> Result<(), CliError> {
    ...
    // Companies
    // =====================================================================
    create_company(conn, "acme-consulting", "Acme Consulting LLC", Some("Demo consulting firm"))?;
    create_company(conn, "acme-products", "Acme Products Inc", Some("Demo product company"))?;
    create_company(conn, "personal", "Personal", Some("Owner personal finances"))?;

    // =====================================================================
    // Chart of Accounts – acme-consulting
    // =====================================================================
    create_account(conn, "acme-consulting", "1000", "Operating Cash", "asset", None)?;
    create_account(conn, "acme-consulting", "1100", "Accounts Receivable", "asset", None)?;
    create_account(conn, "acme-consulting", "1500", "Due from Acme Products", "asset", None)?;
    create_account(conn, "acme-consulting", "2000", "Accounts Payable", "liability", None)?;
    create_account(conn, "acme-consulting", "2500", "Due to Owner", "liability", None)?;
    create_account(conn, "acme-consulting", "3000", "Owner Equity", "equity", None)?;
    create_account(conn, "acme-consulting", "4000", "Consulting Revenue", "revenue", Some("income"))?;
    create_account(conn, "acme-consulting", "5000", "Rent Expense", "expense", Some("rent"))?;
    create_account(conn, "acme-consulting", "5100", "Software Licences", "expense", Some("software"))?;
    create_account(conn, "acme-consulting", "5200", "Office Supplies", "expense", Some("supplies"))?;
    create_account(conn, "acme-consulting", "5300", "Salary Expense", "expense", Some("payroll"))?;
    create_account(conn, "acme-consulting", "2600", "Federal Tax Payable", "liability", Some("payroll-tax"))?;
    create_account(conn, "acme-consulting", "2700", "State Tax Payable", "liability", Some("payroll-tax"))?;
    create_account(conn, "acme-consulting", "2800", "FICA Payable", "liability", Some("payroll-tax"))?;

    // =====================================================================
    // Chart of Accounts – acme-products
    // =====================================================================
    create_account(conn, "acme-products", "1000", "Operating Cash", "asset", None)?;
    create_account(conn, "acme-products", "1100", "Accounts Receivable", "asset", None)?;
    create_account(conn, "acme-products", "1200", "Inventory", "asset", None)?;
    create_account(conn, "acme-products", "1500", "Due from Acme Consulting", "asset", None)?;
    create_account(conn, "acme-products", "2000", "Accounts Payable", "liability", None)?;
    create_account(conn, "acme-products", "3000", "Owner Equity", "equity", None)?;
    create_account(conn, "acme-products", "4000", "Product Sales", "revenue", Some("income"))?;
    create_account(conn, "acme-products", "4100", "Licence Revenue", "revenue", Some("income"))?;
    create_account(conn, "acme-products", "5000", "Cost of Goods Sold", "expense", Some("cogs"))?;
    create_account(conn, "acme-products", "5100", "Shipping Expense", "expense", Some("shipping"))?;
    ...
    create_account(conn, "personal", "1500", "Due from Acme Consulting", "asset", None)?;
    create_account(conn, "personal", "2000", "Credit Card", "liability", None)?;
    create_account(conn, "personal", "3000", "Net Worth", "equity", None)?;
    create_account(conn, "personal", "4000", "Salary Income", "revenue", Some("w2-income"))?;
    create_account(conn, "personal", "4100", "Investment Income", "revenue", Some("investment"))?;
    create_account(conn, "personal", "5000", "Rent", "expense", Some("housing"))?;
    create_account(conn, "personal", "5100", "Groceries", "expense", Some("food"))?;
    create_account(conn, "personal", "5200", "Utilities", "expense", Some("utilities"))?;
    create_account(conn, "personal", "5300", "Federal Tax Withheld", "expense", Some("fed-tax"))?;
    create_account(conn, "personal", "5400", "State Tax Withheld", "expense", Some("state-tax"))?;
    create_account(conn, "personal", "5500", "FICA Withheld", "expense", Some("fica"))?;
    ...
    let c1 = post(
        conn, "acme-consulting", "Owner capital contribution", "USD", "2025-01-01",
    ...
    // C2. Pay January rent
    post(
        conn, "acme-consulting", "January office rent", "USD", "2025-01-05",
    ...
    post(
        conn, "acme-consulting", "Invoice #101 - Globex Corp", "USD", "2025-01-15",
    ...
    post(
        conn, "acme-consulting", "Payment from Globex Corp", "USD", "2025-01-28",
    ...
    post(
        conn, "acme-consulting", "Office supplies - paper and toner", "USD", "2025-02-03",
    ...
    post(
        conn, "acme-consulting", "Invoice #102 - Initech", "USD", "2025-02-15",
    ...
    post(
        conn, "acme-consulting", "February office rent", "USD", "2025-02-05",
        &[dr_tax("5000", 2_500_00, "rent"), cr("1000", 2_500_00)],
        "AC-007", None,
    )?;
    ...
    let p1 = post(
        conn, "acme-products", "Owner capital contribution", "USD", "2025-01-01",
    ...
    post(
        conn, "acme-products", "Initial widget inventory (500 units)", "USD", "2025-01-08",
        &[dr_memo("1200", 5_000_00, "500 widgets @ $10"), cr("1000", 5_000_00)],
        "AP-002", None,
    )?;

    // P3. Sell 100 widgets to retail customer
    post(
        conn, "acme-products", "Widget sale - 100 units", "USD", "2025-01-20",
        &[
            dr("1100", 2_500_00),
            cr_tax("4000", 2_500_00, "income"),
        ],
        "AP-003", None,
    )?;
    ...
    post(
        conn, "acme-products", "COGS - 100 widgets sold", "USD", "2025-01-20",
    ...
    post(
        conn, "acme-products", "Payment received - widget sale", "USD", "2025-02-01",
    ...
    post(
        conn, "acme-products", "Shipping costs - January", "USD", "2025-01-31",
    ...
    post(
        conn, "personal", "Opening balance - savings", "USD", "2025-01-01",
    ...
    post(
        conn, "personal", "Opening balance - checking", "USD", "2025-01-01",
    ...
    post(
        conn, "personal", "January apartment rent", "USD", "2025-01-03",
    ...
    // R4. Groceries
    post(
        conn, "personal", "Weekly groceries", "USD", "2025-01-07",
    ...
    // R5. Utilities
    post(
        conn, "personal", "Electric and internet", "USD", "2025-01-15",
    ...
    post(
        conn, "personal", "February apartment rent", "USD", "2025-02-03",
        &[dr_tax("5000", 1_800_00, "housing"), cr("1000", 1_800_00)],
        "PR-006", None,
    )?;
    ...
    post(
        conn, "personal", "Quarterly dividend - index fund", "USD", "2025-01-31",
    ...
    let ic1_personal = post(
        conn, "personal", "Loan to Acme Consulting", "USD", "2025-01-02",
        &[dr("1500", 5_000_00), cr("1100", 5_000_00)],
        "IC-001-P", None,
    )?;
    let _ic1_consulting = post(
        conn, "acme-consulting", "Loan from owner (personal)", "USD", "2025-01-02",
        &[dr("1000", 5_000_00), cr("2500", 5_000_00)],
        "IC-001-C", Some(ic1_personal),
    )?;
    ...
    let ic2_consulting = post(
        conn, "acme-consulting", "Software licences from Acme Products", "USD", "2025-02-01",
        &[dr_tax("5100", 3_600_00, "software"), cr("2000", 3_600_00)],
        "IC-002-C", None,
    )?;
    let _ic2_products = post(
        conn, "acme-products", "Licence sale to Acme Consulting", "USD", "2025-02-01",
        &[dr("1500", 3_600_00), cr_tax("4100", 3_600_00, "income")],
        "IC-002-P", Some(ic2_consulting),
    )?;
    ...
    let ic3_consulting = post(
        conn, "acme-consulting", "February payroll - owner salary", "USD", "2025-02-15",
        &[
    ...
    let _ic3_personal = post(
        conn, "personal", "February paycheck from Acme Consulting", "USD", "2025-02-15",
        &[
            dr("1000", 3_767_50),
    ...
    let ic4_consulting = post(
        conn, "acme-consulting", "Payment to Acme Products - licence invoice", "USD", "2025-02-20",
        &[dr("2000", 3_600_00), cr("1000", 3_600_00)],
        "IC-004-C", None,
    )?;
    let _ic4_products = post(
        conn, "acme-products", "Payment from Acme Consulting", "USD", "2025-02-20",
        &[dr("1000", 3_600_00), cr("1500", 3_600_00)],
        "IC-004-P", Some(ic4_consulting),
    )?;
...
mod tests {
    ...
    fn demo_intercompany_correlations_are_linked() {
        ...
        let orphans =
            crate::db::find_orphaned_correlations(db.conn()).expect("find orphans");
        assert!(
            orphans.is_empty(),
